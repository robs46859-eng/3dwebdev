...
export async function generateSiteCode(prompt: string): Promise<GeneratedSite> {
  try {
    ...
    if (!text) {
      throw new GeminiError("The AI returned an empty response. This might be due to safety filters or a temporary issue.", "EMPTY_RESPONSE");
    }
    ...
    try {
      ...
      throw new GeminiError("The AI generated an invalid response format. This can happen with complex prompts. Please try again.", "INVALID_JSON", { raw: text });
    }
    ...
    if (prompts.length > 0) {
      ...
      for (const imgPrompt of prompts.slice(0, 3)) {
        try {
          ...
          console.warn(`Image generation failed for prompt: "${imgPrompt.substring(0, 50)}..."`, e.message);
          // We don't throw here to allow the site to still be generated even if some images fail
        }
      }
    }
    ...
    // Handle specific API error codes if possible
    const message = error.message || "An unexpected error occurred during synthesis.";
    
    if (message.includes("quota") || message.includes("429")) {
      throw new GeminiError("API quota exceeded. Please wait a moment and try again.", "QUOTA_EXCEEDED");
    }
    
    if (message.includes("safety") || message.includes("blocked")) {
      throw new GeminiError("The prompt was flagged by safety filters. Please try a different description.", "SAFETY_BLOCKED");
    }

    if (message.includes("network") || message.includes("fetch")) {
      throw new GeminiError("Network error. Please check your connection and try again.", "NETWORK_ERROR");
    }
